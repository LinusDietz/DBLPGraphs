...
def printSFDPa(database, author):
    ...
    sfdpFile += '}'
    sfdpFileName = outputPath("coadb_connected_", urllib.unquote(author))

    f = open(sfdpFileName, 'wb')
    f.write(urllib.unquote(sfdpFile))
    f.close()
...
def printSFDPaWeigths(database, author):
    ...
    for relAuthor in relAuthors:
        for coAuthor in database.coauthorsDB[relAuthor]:
            if coAuthor in relAuthors:
                sfdpFile += dotFileEsc(relAuthor) + ' -- ' + dotFileEsc(coAuthor) + ' [label=\"' + str(database.coauthorsDB[relAuthor][coAuthor]) + '\"];\n'
...
def printSFDP2PDF(database, author):
    ...
    sfdpFile += line 
    sfdpFile += '}'
    sfdpFileName = outputPath("coadb_coauthors_", urllib.unquote(author))
